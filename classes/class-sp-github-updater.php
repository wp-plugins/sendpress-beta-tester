...
if ( !class_exists( 'SP_GitHub_Updater' ) ):

	require_once( 'class-wpgithubupdater.php' );

	class SP_GitHub_Updater extends WPGitHubUpdater {
		...
		public function __construct( $config = array() ) {
			...
			$plugin_folder = 'sendpress';
			$plugin_file = 'sendpress.php';
			$github_path = 'brewlabs/SendPress';

			$this->config = array(
				'slug' => trailingslashit( $plugin_folder ) . $plugin_file,
				'proper_folder_name' => $plugin_folder,
				'api_url' => 'https://api.github.com/repos/' . $github_path,
				'raw_url' => 'https://raw.github.com/' . $github_path . '/master',
				'github_url' => 'https://github.com/' . $github_path,
				'zip_url' => 'https://github.com/' . $github_path . '/zipball/master',
				'readme' => 'readme.txt',
				'sslverify' => true,
				'requires' => $wp_version,
				'tested' => $wp_version
			...
			$this->config['version'] = get_option( 'sp_beta_tester_installed_commit', false );
			$this->config['new_version'] = $this->get_latest_commit()->sha;

			$this->set_defaults();

			if ( ( defined('SP_GITHUB_FORCE_UPDATE') && SP_GITHUB_FORCE_UPDATE ) || ( defined('WP_GITHUB_FORCE_UPDATE') && WP_GITHUB_FORCE_UPDATE ) )
